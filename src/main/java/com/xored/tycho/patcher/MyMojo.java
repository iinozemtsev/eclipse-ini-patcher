    ...
    extends AbstractMojo
{
        ...
        throws MojoExecutionException
    {
    	...
    	Plugin plugin = project.getPlugin( "org.sonatype.tycho:tycho-p2-director-plugin" );
    	if(plugin == null) {
	...
	private void patchProduct(String productId) {
		...
		File iniFile = new File(macDir, "Eclipse.app/Contents/MacOS/eclipse.ini");
		...
		try {
			...
			patched.append(String.format("../../../plugins/%s", launcherFile.getName())).append("\n");
	...
	private File findLauncher(File macDir) {
		...
		for(File file : plugins.listFiles()) {
			if(file.isFile() &&
					file.getName().startsWith(LAUNCHER_JAR) &&
